...
static ID3D11ShaderResourceView* g_testTex = NULL;
	...
	:Enemy(ENEMY_DYNAMIC_LIFE, ENEMY_DYNAMIC_DAMAGE, ENEMY_DYNAMIC_SOULGAGE, ENEMY_DYNAMIC_SCORE, true, false)
{
	...
	shape2.m_radius = size.x * 0.5f;

	b2FixtureDef fixture2;
	fixture2.shape = &shape2;
	fixture2.density = 1.0f;
	fixture2.friction = 0.1f;//摩擦
	...
	b2Fixture* enemy_dynamic_fixture = GetBody()->CreateFixture(&fixture2);//Bodyにフィクスチャを登録する
...
void EnemyDynamic::Initialize()
{
	...
	g_testTex = InitTexture(L"asset\\texture\\sample_texture\\sample_rock.png");
}
...
void EnemyDynamic::Update()
{
	...
	else if (!GetUse())
	{
		...
		{
			object_manager.FindEnemyAttackByID(GetID())->Finalize();
		}
...
//============================================================
	...
	//貼るテクスチャを指定
	if (GetState() == ENEMY_STATE_MOVE)
	{
		GetDeviceContext()->PSSetShaderResources(0, 1, &g_EnemyDynamic_Texture);
	}
	else if (GetState() == ENEMY_STATE_ATTACK)
	{
		GetDeviceContext()->PSSetShaderResources(0, 1, &g_EnemySensor_Texture);
	}
	else
	{
		GetDeviceContext()->PSSetShaderResources(0, 1, &g_testTex);
	}
...
{	
	...
	if (GetBody()->GetLinearVelocity() == b2Vec2(0.0, 0.0) && m_old_state == ENEMY_STATE_MOVE)
	{
		GetBody()->SetLinearVelocity(b2Vec2(0.0, 0.0));
		if (GetDirection())
		{

			GetBody()->ApplyLinearImpulseToCenter(b2Vec2(-m_speed, -0.5), true);
		}
		else
		{
			GetBody()->ApplyLinearImpulseToCenter(b2Vec2(m_speed, -0.5), true);
...
void EnemyDynamic::Attack()
{
	...
	if (m_attack_counter == m_attack_birth)
	{
		...
		if (GetDirection())
		{
			object_manager.AddEnemyAttack(b2Vec2(pos.x - GetSize().x / 5, pos.y), GetSize(), 0.0f, GetID());
		}
		else
		{
			object_manager.AddEnemyAttack(b2Vec2(pos.x + GetSize().x / 5, pos.y), GetSize(), 0.0f, GetID());
