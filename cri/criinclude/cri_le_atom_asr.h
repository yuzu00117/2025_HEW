...
/*JP
 * \brief マトリクスの入出力チャンネルがデフォルトで指定された場合の動作時パラメタ数
 * \ingroup CRIATOMASR_DSP_PARAM_MATRIX
 * \par 説明:
 ...
 * \ingroup ATOMEXLIB_ASR
 * \return	CriSint32	ASRラックID
 * \par 説明:
 * Ambisonics再生のみを行うASRラックのIDを取得します。<br>
 * 作成されていない場合、CRIATOMEXASR_RACK_ILLEGAL_IDが返されます。
 */
 ...
 * \par 備考:
 * 但し、 ::criAtomExPlayer_AddOutputPort 関数または ::criAtomExPlayer_AddPreferredOutputPort 関数を使用して
 * プレーヤーに追加中の出力ポートハンドルは破棄することができません。<br>
 * ::criAtomExPlayer_RemoveOutputPort 関数または ::criAtomExPlayer_RemovePreferredOutputPort 関数を使用して
 * プレーヤーから取り外してから破棄してください。<br>
 * また、ACFファイルの情報から作成されたACF内の出力ポートハンドルは破棄することができません。
 ...
 * \return	CriSint32	ASRラックID
 * \par 説明:
 * チャンネルベース 再生のみを行うASRラックのIDを取得します。<br>
 * 作成されていない場合、CRIATOMEXASR_RACK_ILLEGAL_IDが返されます。
 */
 ...
 * \return	CriSint32	ASRラックID
 * \par 説明:
 * ObjectBasedAudio 再生のみを行うASRラックのIDを取得します。<br>
 * 作成されていない場合、CRIATOMEXASR_RACK_ILLEGAL_IDが返されます。
 */
