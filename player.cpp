...
// たまちゃんが消えるときのエフェクト
static ID3D11ShaderResourceView *g_tamachan_effect_lv1 = NULL; // たまちゃんLv1のエフェクト
static ID3D11ShaderResourceView *g_tamachan_effect_lv2 = NULL; // たまちゃんLv2のエフェクト
static ID3D11ShaderResourceView *g_tamachan_effect_lv3 = NULL; // たまちゃんLv3のエフェクト
...
static b2Vec2 g_Beziers_prev_position;            // 前フレームでのposition
...
void Player::Initialize(b2Vec2 position, b2Vec2 body_size, b2Vec2 sensor_size)
{
    ...
    if (g_player_Texture == NULL)
    {
        ...
        // たまちゃんが消えるときのエフェクト
        g_tamachan_effect_lv1 = InitTexture(L"asset\\texture\\anchor_point\\EFF_Tama_Blue_5x10.png");
        g_tamachan_effect_lv2 = InitTexture(L"asset\\texture\\anchor_point\\EFF_Tama_Yellow_5x10.png");
        g_tamachan_effect_lv3 = InitTexture(L"asset\\texture\\anchor_point\\EFF_Tama_Red_5x10.png");
...
void Player::Update()
{
    ...
    case Create_wait_draw_cnt_state:
        ...
        // アンカーを投げたフラグを立てる（たまちゃんを消す用）
        is_throw_anchor = true;
    ...
    case Deleting_state: // 削除している状態
        ...
        // アンカーを投げたフラグを下ろす（たまちゃんを消す用）
        is_throw_anchor = false;
...
void Player::DrawDustEffect()
{
    ...
    {
        for (size_t i = 0; i < dustEffects.size() - 1; i++)
        { // 最後の要素はスキップ
        ...
        if (Anchor_level3_Frame_Sheet_cnt < 50)
        {
            ...
            DrawDividedSprite(
                {SCREEN_WIDTH / 2,
                 SCREEN_HEIGHT / 2},
                0,
                {1280, 720}, 10, 5, Anchor_level3_Frame_Sheet_cnt, 0.5f);
...
void Player::DrawTamaChan()
{
    ...
    // たまちゃんの描画位置を計算
    tamachan_pos_x = ((tamachan_pos.x - PlayerPosition::GetPlayerPosition().x) * BOX2D_SCALE_MANAGEMENT) * scale + screen_center.x;
    tamachan_pos_y = (((tamachan_pos.y - PlayerPosition::GetPlayerPosition().y) * BOX2D_SCALE_MANAGEMENT) * scale + screen_center.y) - 100;

    // スプライトを描画
    // bool is_throw_anchorがtrueの時は、たまちゃんを描画しない
    if (!is_throw_anchor)
    {
        // アンカーレベルに応じてたまちゃんのサイズを変更
        switch (AnchorSpirit::GetAnchorLevel())
        {
        case 1:
            // シェーダリソースを設定
            GetDeviceContext()->PSSetShaderResources(0, 1, &g_TamaChan_Lv1);
            tamachan_size = {75.f, 75.f}; // たまちゃんのサイズ
            break;
        case 2:
            // シェーダリソースを設定
            GetDeviceContext()->PSSetShaderResources(0, 1, &g_TamaChan_Lv2);
            tamachan_size = {125.f, 125.f}; // たまちゃんのサイズ
            break;
        case 3:
            // シェーダリソースを設定
            GetDeviceContext()->PSSetShaderResources(0, 1, &g_TamaChan_Lv3);
            tamachan_size = {175.f, 175.f}; // たまちゃんのサイズ
            break;
        default:
            break;
        }
